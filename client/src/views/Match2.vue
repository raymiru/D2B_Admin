...
<script>
    ...
    export default {
        ...
        created() {
            bus.$on('match2', () => {
                if (localStorage['match2-players-ids']) {
                    let matchPlayersIds = JSON.parse(localStorage['match2-players-ids']);
                    ...
                    localStorage['match2-active-players'] = JSON.stringify(this.active_players)
            ...
            if (localStorage['match2-active-players']) {
                this.active_players = JSON.parse(localStorage['match2-active-players']);
            ...
            winner: function (data) {
                ...
                if (data == 1) {
                    this.$socket.emit('msg_to_watcher', {
                        steam_username: JSON.parse(localStorage['match2-watcher'])[0].steam_username,
                        team_winner: data,
                    })
                } else if (data == 2) {
                    this.$socket.emit('msg_to_watcher', {
        ...
        sockets: {
            ...
            bet_msg_from_watcher: function (data) {
                try {
                    if (data.player_id == JSON.parse(localStorage['match2-watcher-id'])[0]) {
                    ...
                    console.log('match2-watcher-id LS отсутствует')
                }
